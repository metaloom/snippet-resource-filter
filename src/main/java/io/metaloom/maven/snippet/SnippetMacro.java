...
public class SnippetMacro {
	...
	public String execute(Map<String, String> request, File baseDir) {
		String id = (String) request.get("id");

		String urlParam = (String) request.get("url");

		String fileParam = (String) request.get("file");

		String debugParam = (String) request.get("debug");

		if (debugParam != null) {
			this.debug = Boolean.parseBoolean(debugParam);
		}

		String ignoreDownloadErrorParam = (String) request.get("ignoreDownloadError");
		...
		try {
			...
			throw new RuntimeException("Error reading snippet", e);
		}
	 ...
	 *            The URL to parse.
	 * @param encoding
	 *            The encoding of the URL to parse.
	 * @param id
	 ...
	 * @param id
	 *            The id of the snippet.
	 * @return An identifier, concatenated url and id, or just url.toString() if id is empty or null.
	 */
