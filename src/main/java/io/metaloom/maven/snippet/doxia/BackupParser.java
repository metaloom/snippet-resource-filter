...
@Component(role = Parser.class, hint = "dgasdg")
public class BackupParser extends AbstractTextParser implements TextMarkup {
	...
	private static final int RAW_BLOCK = 2;
	 ...
	 *
	 * @param firstBlock
	 *            True if this is the first block of the Apt source document.
	 * @throws AptParseException
	 *             if something goes wrong.
	 */
	...
	private abstract class Block {
		...
		Block(int type, int indent, String firstLine)
			...
			if (firstLine == null) {
				...
				while (BackupParser.this.line != null) {
					String l = BackupParser.this.line;
					int length = l.length();
					int i = 0;
	...
	private class RawBlock
		 ...
		 *             AptParseException
		 */
		RawBlock(String line)
			throws AptParseException {
			super(RAW_BLOCK, 0, line);
		}

		/** {@inheritDoc} */
		public void traverse() throws AptParseException {
			BackupParser.this.sink.rawText(text);
		}
	}
	...
	private class MacroBlock
		...
		public void traverse()
			...
			MacroRequest request = new MacroRequest(sourceContent, new BackupParser(), parameters, getBasedir());
			try {
				BackupParser.this.executeMacro(macroId, request, sink);
